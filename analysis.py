...
def generate_nasa_report(summary, raw_data):
    ...
    <head>
        ...
        <style>
            :root {{
                --bg-main: #0f172a; --bg-panel: #1e293b; --border: #334155;
                --text-main: #f8fafc; --text-muted: #94a3b8;
                --cyan: #38bdf8; --red: #f43f5e; --yellow: #fbbf24; --green: #10b981;
            ...
            * {{ box-sizing: border-box; margin: 0; padding: 0; }}
            ...
            header {{ display: flex; justify-content: space-between; align-items: center; padding: 20px 40px; background: var(--bg-panel); border-bottom: 1px solid var(--border); }}
            .logo-area h1 {{ font-size: 1.5rem; font-weight: 600; letter-spacing: 1px; color: var(--text-main); display: flex; align-items: center; gap: 10px; }}
            .logo-dot {{ width: 12px; height: 12px; background: var(--cyan); border-radius: 50%; display: inline-block; }}
            .logo-area .subtitle {{ font-family: 'JetBrains Mono', monospace; font-size: 0.8rem; color: var(--text-muted); margin-top: 4px; display: flex; align-items: center; gap: 8px; }}
            .live-indicator {{ display: inline-block; width: 8px; height: 8px; background: var(--red); border-radius: 50%; animation: pulse 2s infinite; }}
            .clock {{ font-family: 'JetBrains Mono', monospace; font-size: 1.1rem; color: var(--text-muted); }}
            .dashboard {{ display: grid; grid-template-columns: 280px 1fr; gap: 30px; padding: 30px 40px; height: calc(100vh - 120px); }}
            .sidebar {{ display: flex; flex-direction: column; gap: 20px; }}
            .status-card {{ background: var(--bg-panel); border: 1px solid var(--border); border-radius: 12px; padding: 20px; display: flex; flex-direction: column; gap: 10px; }}
            .status-card h3 {{ color: var(--text-muted); font-size: 0.85rem; text-transform: uppercase; letter-spacing: 1px; font-weight: 500; }}
            .status-card .value {{ font-family: 'JetBrains Mono', monospace; font-size: 2rem; font-weight: 500; }}
            .radar-box {{ height: 200px; display: flex; align-items: center; justify-content: center; background: var(--bg-panel); border: 1px solid var(--border); border-radius: 12px; position: relative; overflow: hidden; }}
            .radar-circle {{ width: 120px; height: 120px; border: 1px dashed var(--border); border-radius: 50%; position: relative; display: flex; align-items: center; justify-content: center; }}
            .radar-circle::after {{ content: ''; width: 40px; height: 40px; background: rgba(56, 189, 248, 0.1); border: 1px solid rgba(56, 189, 248, 0.3); border-radius: 50%; animation: ripple 3s infinite ease-out; }}
            .radar-dot {{ position: absolute; width: 6px; height: 6px; background: var(--cyan); border-radius: 50%; }}
            .news-feed {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 20px; overflow-y: auto; padding-right: 10px; padding-bottom: 20px; align-content: start; }}
            ...
            .news-card {{ background: var(--bg-panel); border: 1px solid var(--border); border-radius: 12px; padding: 20px; display: flex; flex-direction: column; justify-content: space-between; transition: transform 0.2s ease, border-color 0.2s ease; }}
            .news-card:hover {{ transform: translateY(-2px); border-color: var(--text-muted); }}
            .card-meta {{ display: flex; justify-content: space-between; align-items: center; font-family: 'JetBrains Mono', monospace; font-size: 0.75rem; color: var(--text-muted); margin-bottom: 12px; }}
            .time-badge {{ background: var(--bg-main); padding: 4px 8px; border-radius: 6px; border: 1px solid var(--border); }}
            .card-title {{ font-size: 1.05rem; font-weight: 500; margin-bottom: 10px; line-height: 1.4; color: var(--text-main); }}
            .card-desc {{ font-size: 0.85rem; color: var(--text-muted); line-height: 1.5; }}
            .threat-indicator {{ display: inline-flex; align-items: center; gap: 6px; font-family: 'JetBrains Mono', monospace; font-size: 0.75rem; padding-top: 15px; margin-top: 15px; border-top: 1px solid var(--border); }}
            .threat-dot {{ width: 8px; height: 8px; border-radius: 50%; }}
            .ticker-wrap {{ position: fixed; bottom: 0; left: 0; width: 100%; height: 40px; background: var(--bg-panel); border-top: 1px solid var(--border); display: flex; align-items: center; z-index: 1000; }}
            .ticker-title {{ background: var(--bg-main); color: var(--text-muted); padding: 0 20px; font-weight: 500; font-size: 0.8rem; height: 100%; display: flex; align-items: center; z-index: 20; border-right: 1px solid var(--border); text-transform: uppercase; letter-spacing: 1px; }}
            .ticker-marquee {{ flex: 1; overflow: hidden; position: relative; height: 100%; display: flex; align-items: center; }}
            .ticker-move {{ display: flex; white-space: nowrap; padding-left: 100%; animation: ticker 250s linear infinite; }}
            .ticker-move:hover {{ animation-play-state: paused; }}
            .ticker-item {{ margin-right: 60px; color: var(--text-main); font-size: 0.9rem; display: inline-flex; align-items: center; gap: 10px; }}
    ...
    <body>
        ...
        <script>
            ...
            let tickerHTML = '';
            const colors = {{ 'HIGH': 'var(--red)', 'MEDIUM': 'var(--yellow)', 'LOW': 'var(--cyan)' }};
...
if __name__ == "__main__":
    ...
    # === זה החלק שמחזיר לך את הסיכום היפה ב-GitHub Actions ===
